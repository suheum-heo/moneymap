...
export default function Overview({ entries, month }: Props) {
  ...
  const fmt = (n: number, symbol: string) =>
    `${symbol}${Math.abs(n).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`

  const fmtConverted = (n: number) =>
    showConversion ? ` (≈${homeSym}${convert(Math.abs(n), cur, homeCur).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })})` : ''

  return (
    <div className="px-4 pb-8">
      <div className="grid grid-cols-3 gap-2 mb-5">
        {[
          { label: 'Expenses', value: fmt(expenses, sym), sub: fmtConverted(expenses), color: 'text-red-600 dark:text-red-400' },
          { label: 'Income', value: fmt(income, sym), sub: fmtConverted(income), color: 'text-green-700 dark:text-green-400' },
          { label: 'Net', value: (net < 0 ? '-' : '') + fmt(net, sym), sub: fmtConverted(net), color: net < 0 ? 'text-red-600 dark:text-red-400' : 'text-green-700 dark:text-green-400' },
        ].map(m => (
