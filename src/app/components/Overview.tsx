...
export default function Overview({ entries, month, onNavigate }: Props) {
  ...
  const chartGridColor = isDark ? 'rgba(148,163,184,0.18)' : 'rgba(148,163,184,0.22)'
  ...
  useEffect(() => {
    ...
    locChartInstance.current = new Chart(locChartRef.current, {
      ...
      data: {
        ...
        datasets: [{ data: byLocation.map(([, v]) => parseFloat(v.toFixed(2))), backgroundColor: '#6e80ff', borderRadius: 10, borderSkipped: false }]
      },
      options: {
    ...
    <div className="px-4 pb-8">
      <div className="grid gap-3 sm:grid-cols-3">
        {[
          ...
          { label: t('net'), value: (net < 0 ? '-' : '') + fmt(net), sub: fmtHome(netHome), color: net < 0 ? 'text-rose-500 dark:text-rose-300' : 'text-indigo-500 dark:text-indigo-300', filter: 'all' },
        ].map(m => (
          <button
            key={m.label}
            onClick={() => onNavigate('entries', m.filter)}
            className="app-panel flex flex-col items-start p-5 text-left transition-transform hover:-translate-y-0.5"
          >
            <span className="app-kicker mb-3">{m.label}</span>
            <span className={`text-2xl font-semibold tracking-tight ${m.color}`}>{m.value}</span>
            {m.sub && <span className="mt-2 text-sm text-zinc-400">{m.sub}</span>}
          </button>
        ))}
      </div>

      {lastMonthExpenses > 0 && (
        <div className="app-panel-soft mt-4 px-4 py-4 sm:px-5">
          <div className="app-kicker mb-3">{t('vsLastMonth')}</div>
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="text-zinc-500 dark:text-zinc-300">{t('vsLastMonth')}</span>
            ...
            {isCurrentMonth && sameDayLastMonth > 0 && (
              <div className="flex items-center justify-between gap-3 text-sm">
                <span className="text-zinc-500 dark:text-zinc-300">{t('vsSameDay')}</span>
      ...
      {monthEntries.length === 0 && (
        <div className="app-panel-soft mt-6 py-14 text-center text-sm text-zinc-400">{t('noEntries')}</div>
      )}
          ...
          <div className="app-panel p-5 sm:p-6">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">{t('expenses')}</h3>
              </div>
              <div className="text-right text-xs text-zinc-400">{byCategory.length} {byCategory.length === 1 ? 'category' : 'categories'}</div>
            </div>
                ...
                return (
                  <div key={rowIdx} className="space-y-2">
                      ...
                      {row.map(([cat, amt]) => {
                        ...
                        return (
                          <button
                            ...
                            className="app-panel-soft rounded-[24px] px-4 py-4 text-left transition-transform hover:-translate-y-0.5"
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                  <span className="h-2.5 w-2.5 rounded-full flex-shrink-0" style={{ background: col }} />
                                  <span className="truncate text-sm font-medium text-zinc-800 dark:text-zinc-100">{cat}</span>
                                ...
                                <div className="mt-2 text-xs text-zinc-400">{pct}% of spending</div>
                              </div>
                              <div className="text-right">
                                <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">{formatAmount(amt, cur)}</div>
                                {budget && <div className="mt-1 text-xs text-zinc-400">/ {formatAmount(budget, cur)}</div>}
                                <div className="mt-2 text-xs text-zinc-400">{isExpanded ? '▲' : '▼'}</div>
                              </div>
                            </div>
                            {budget && (
                    ...
                    {expandedInRow && (() => {
                      ...
                      return (
                        <div className="rounded-[24px] border border-zinc-200/70 bg-white/70 px-3 py-3 dark:border-white/10 dark:bg-slate-950/50">
                          <div className="space-y-2">
                            {catEntriesForCat.map(e => (
                              ...
                              >
                                <div className="w-10 flex-shrink-0 text-xs text-zinc-400">{e.date.slice(5)}</div>
                                <div className="min-w-0 flex-1">
                                  <div className="truncate text-sm font-medium text-zinc-800 dark:text-zinc-100">{e.summary}</div>
                                  {e.venue && <div className="truncate text-xs text-zinc-400">{e.venue}{e.location ? ` · ${e.location}` : ''}</div>}
                                </div>
                          ...
                          <div className="px-2 pt-3 text-xs text-zinc-400">
          ...
          {byCategory.length > 0 && (
            <div className="app-panel p-5 sm:p-6">
              <div className="mb-4">
                ...
                <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">{t('spendingChart')}</h3>
              </div>
          ...
          <div className="app-panel p-5 sm:p-6">
            ...
            <div className="space-y-3">
              {byLocation.map(([loc, amt]) => {
                const pct = expenses > 0 ? ((amt / expenses) * 100).toFixed(1) : '0'
                return (
                  <div key={loc} className="rounded-[24px] border border-zinc-200/70 bg-white/60 px-4 py-4 dark:border-white/10 dark:bg-slate-950/45">
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <span className="truncate text-sm font-medium text-zinc-800 dark:text-zinc-100">{loc}</span>
                      <span className="flex-shrink-0 text-xs text-zinc-400">{pct}%</span>
                    </div>
                    <div className="h-2 overflow-hidden rounded-full bg-zinc-200/70 dark:bg-white/10">
                      <div className="h-full rounded-full bg-indigo-400" style={{ width: `${pct}%` }} />
                    </div>
                    <div className="mt-3 text-right text-sm font-semibold text-zinc-900 dark:text-zinc-50">{formatAmount(amt, cur)}</div>
                  </div>
                )
              })}
            </div>
