    ...
    <div className="px-4 pb-8">
      {editEntry && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center p-4" onClick={() => setEditEntry(null)}>
          <div className="bg-[#fafaf8] dark:bg-[#1a1a18] rounded-2xl p-4 w-full max-w-md flex flex-col gap-3" onClick={e => e.stopPropagation()}>
            ...
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">{t('amount')} ({cur})</label>
      ...
      <div className="flex gap-2 mb-3 flex-wrap items-center">
        ...
        <button onClick={() => setWeekOnly(v => !v)}
          className={`text-sm px-3 py-1.5 rounded-lg border transition-colors ${weekOnly ? 'bg-amber-500 text-white border-amber-500' : 'border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400'}`}>
          {t('thisWeek')}
        </button>
      ...
      {weekOnly && weekTotal !== null && (
        <div className="mb-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-xl px-3 py-2 flex justify-between items-center">
          <span className="text-xs text-amber-700 dark:text-amber-400">{t('thisWeek')} ({weekRange.start.slice(5)} – {weekRange.end.slice(5)})</span>
          <span className="text-sm font-medium text-red-600 dark:text-red-400">-{formatAmount(weekTotal, cur)}</span>
        </div>
      )}

      {filtered.length === 0 ? (
        <div className="text-center text-zinc-400 py-12 text-sm">{t('noEntriesFound')}</div>
      ) : (
            ...
            return (
              <div key={e.id} className="flex items-start gap-3 rounded-xl px-3 py-2.5" style={{ background: col + '14', borderLeft: `3px solid ${col}` }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="inline-block text-xs px-2 py-0.5 rounded-full mt-1" style={{ background: col + '25', color: col }}>{e.category}</span>
                </div>
                <div className="flex flex-col items-end gap-0.5">
                  ...
                  <div className="flex gap-2 mt-1">
                    <button onClick={() => openEdit(e)} className="text-xs text-amber-500 hover:text-amber-600">{t('edit')}</button>
                    {confirmId === e.id ? (
                      <>
