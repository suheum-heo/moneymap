      ...
      {editEntry && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center p-4" onClick={() => setEditEntry(null)}>
          <div className="bg-[#fafaf8] dark:bg-[#1a1a18] rounded-2xl p-4 w-full max-w-md flex flex-col gap-3" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              {(['expense', 'income'] as const).map(t => (
                <button key={t} onClick={() => { setEditType(t); setEditCategory(t === 'expense' ? EXPENSE_CATEGORIES[3] : INCOME_CATEGORIES[0]) }}
                  className={`flex-1 py-1.5 rounded-xl text-sm font-medium border transition-colors ${editType === t ? 'bg-amber-500 text-white border-amber-500' : 'bg-transparent border-zinc-200 dark:border-zinc-700 text-zinc-500'}`}>
            ...
            <button onClick={handleSave} className="w-full py-2.5 rounded-xl bg-amber-500 text-white text-sm font-medium mt-1">Save changes</button>
              ...
              <div key={e.id} className="flex items-start gap-3 rounded-xl px-3 py-2.5" style={{ background: col + '14', borderLeft: `3px solid ${col}` }}>
                ...
                <div className="flex flex-col items-end gap-0.5">
                  ...
                  <div className="flex gap-2 mt-1">
                    <button onClick={() => openEdit(e)} className="text-xs text-amber-500 hover:text-amber-600">Edit</button>
                    {confirmId === e.id ? (
                      <>
